    ...
    try {
        const pipeline = [
            ...
            // Lookup Abonos para Facturas
            {
                $lookup: {
                    from: 'abonos',
                    let: { facturas_ids: '$facturas._id' },
                    pipeline: [
                        { 
                            $match: { 
                                $expr: { 
                                    $in: ['$factura_id', '$$facturas_ids'] 
                                } 
                            } 
                        },
                        { 
            ...
            // Agregar los campos necesarios
            {
                $addFields: {
                    ...
                    // Resumen Honorarios
                    cantidadHonorarios: { $size: '$honorarios' },
                    montoHonorariosTotal: { 
                        $sum: { 
            ...
            // Proyectar campos que no necesitamos
            {
                $project: {
                    ...
                    montoFacturasTotal: 0,
                    montoHonorariosTotal: 0
                }
            },
            // Ordenar
            { 
